...
//
// vmm.c for Simple-XX/SimpleKernel.
...
// 内核页表区域
static pte_t pte_kernel[VMM_PAGE_TABLES_KERNEL][VMM_PAGES_PRE_PAGE_TABLE]
    __attribute__((aligned(VMM_PAGE_SIZE)));
...
void vmm_init(void) {
    ...
    for (uint32_t i = pgd_idx, j = 0; i < VMM_PAGE_DIRECTORIES_KERNEL + pgd_idx;
         i++, j++) {
        pgd_kernel[i] = ((ptr_t)vmm_la_to_pa((ptr_t)pte_kernel[j]) |
...
>>>>>>> vmm_and_pmm
    ...
    for (uint32_t i = 0; i < VMM_PAGE_TABLES_KERNEL * VMM_PAGES_PRE_PAGE_TABLE;
         i++) {
        // 物理地址由 (i << 12) 给出
