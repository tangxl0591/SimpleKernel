...
void multiboot2_init(ptr_t magic, ptr_t addr) {
    ...
    for (tag = (multiboot_tag_t *)tag_addr; tag->type != MULTIBOOT_TAG_TYPE_END;
        ...
        case MULTIBOOT_TAG_TYPE_ELF_SECTIONS: {
            ...
            // TODO
            // Elf64_Shdr * shdr = (Elf64_Shdr*)( (uint32_t)tag + sizeof( struct
            // multiboot_tag_elf_sections ) ); uint32_t shstrtab = shdr[(
            // (struct multiboot_tag_elf_sections*)tag )->shndx].sh_addr;
            ...
            // multiboot_tag_elf_sections*)tag )->num; i++) { 	const char *name =
            // (const char *)(shstrtab + shdr[i].sh_name);
            // 	// printk(" sh_name: %s ", name); // correct
            // 	// printk("shaddr: 0x%X\t", shdr[i].sh_addr); // correct
            // 	// 在 GRUB 提供的 multiboot 信息中寻找内核 ELF
            // 格式所提取的字符串表和符号表 	if (strcmp(name, ".strtab") == 0) {
